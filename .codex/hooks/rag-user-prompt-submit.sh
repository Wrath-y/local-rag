#!/bin/bash
# Codex UserPromptSubmit hook — fail open so RAG availability never blocks a prompt.

PAYLOAD=$(cat)

# Disabled mode and empty prompts are bypasses, not hook outcomes. The server
# makes the same check; this local guard avoids client-failure telemetry for a
# deliberate bypass while the service is unavailable.
...
# This telemetry is best effort: it must not delay or alter the prompt path.
report_outcome() {
  local outcome="$1"
  local reason_code="$2"
  (
...
if [ -n "$CTX" ]; then
  jq -cn --arg context "$CTX" '{hookSpecificOutput:{hookEventName:"UserPromptSubmit",additionalContext:$context}}'
fi
