...
| Tool | 说明 | 参数 |
|------|------|------|
| `rag_ingest` | 写入文本或连接器来源 | `text` 或显式 `kind` + `path`/`url`；可选 `source`、Git `ref`/`exclusions` 和只能收紧上限的 `limits` |
...
除直接文本外，支持 `txt`、`json`、`pdf`、`docx`、`web_url` 和 `git`。Git 远程地址必须是无凭据 HTTPS/SSH URL，认证仅通过主机 credential helper 或 SSH agent。
...
响应：`Ingested N chunks.`（`N` 由当前分块策略决定。）
