...
📝 [本地检索反馈](docs/retrieval-feedback.md)
...
> 🔒 使用默认的本地 embedding 和 rerank 提供方时，数据库和模型推理均在本机完成。
> 如配置 OpenAI、Cohere、Jina、OpenRouter 或其他远程提供方，则该提供方处理
> embedding、rerank 或 LLM 请求所需的文本会发送至对应服务。
...
| 工具 | 说明 |
|------|------|
| [Go 1.25.4+](https://go.dev/dl/) | 编译服务端（与 `go.mod` 一致） |
...
`citations`、`evidence_token` 和持久化的 `retrieval_id`；每个结构化引用还带有
仅在该次检索中有效的 `citation_id`。可将最终回答提交至
`POST /citations/validate`，校验有效、伪造或缺失的引用。完整字段、时效及不确定性
处理方式见 [引用契约](docs/CITATIONS.md)。

### 📝 记录检索反馈

检索完成后，可使用 `retrieval_id` 记录 `helpful`、`unhelpful` 或
`citation-error` 反馈；引用错误必须带回该次结果中的一个或多个 `citation_id`：

```bash
curl -X POST http://127.0.0.1:8765/feedback \
  -H 'Content-Type: application/json' \
  -d '{"retrieval_id":"<retrieval-id>","kind":"citation-error","citation_ids":["<citation-id>"]}'
```

反馈、查询指纹与引用快照仅保存在本地。默认不保存查询明文摘录，备注和摘录也不会
默认出现在列表或导出中。使用 `/feedback`、`/feedback/aggregate`、
`/feedback/export` 分析本地数据；通过 `/feedback/candidates/convert` 显式创建待审核
的离线评测候选。反馈和已批准候选绝不会改变在线检索、重排、向量、索引或模型行为。
完整 HTTP/MCP 契约与隐私/保留策略见[本地检索反馈文档](docs/retrieval-feedback.md)。
...
> 使用 `rerank.provider: local` 时，首次开启会下载
> `BAAI/bge-reranker-base`（约 400 MB），之后在进程内复用。每次检索通常额外
> 增加约 50～200 ms，不消耗模型上下文 token。远程 rerank 提供方使用各自的模型、
> 延迟、计费和数据处理策略。
...
编辑 `config.yaml` 调整参数。以下为有效配置示例；未填写的字段由服务端使用默认值。

```yaml
server:
  port: 8765

# Embedding 提供方: "local"（Python 微服务）、"openai" 或 "custom"
embedding:
  provider: "local"
...
# Rerank: "local" / "cohere" / "jina" / "custom" / "disabled"
rerank:
  provider: "local"
...
# Python sidecar 设置（仅 embedding.provider: local 时使用）
sidecar:
  port: 8766

# 可选的查询改写；需要可用的 llm 提供方
query_rewrite:
  enabled: false
...
完整支持的字段以
[`internal/config/config.go`](internal/config/config.go) 为准。其中，来源增量同步配置位于
`sync`，反馈保留策略位于 `feedback`，本地 sidecar 健康检查配置位于 `sidecar`。
`storage.index_path`、`storage.texts_path` 和 `rerank.enabled` 等字段当前不会被服务端使用。
...
无需 Python 环境，配置 `embedding` 后在启动服务前导出对应 API Key，即可直接调用远端 API：
...
`openai` 和 `custom` embedding 提供方的 `base_url` 默认为 OpenAI API 地址。远程
embedding 提供方会接收其处理所需的文档 chunk 和检索查询。
...
运行时可切换查询改写：
```bash
/rag-rewrite on    # 开启查询改写
```

请在启动服务前通过 `config.yaml` 选择分块策略。修改后需重启服务；已有 chunk
不会自动重新分块。
...
| `/rag-reset` | 清空全部知识库 | — |
| `/rag-export [path]` | 导出 ZIP 数据库备份 | — |
| `/rag-import <path>` | 从 ZIP 备份导入（会请求确认） | — |
